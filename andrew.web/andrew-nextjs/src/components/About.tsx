...
export default function About() {
  ...
  return (
    <section id="about" className="about container">
      <div className="about-main grid grid-cols-[0.6fr_1.4fr] gap-9 items-center mb-12">
        <div 
          ref={photoRef.ref} 
        ...
        >
          <h2 className="text-3xl m-0 mb-3.5 font-black">Quién soy</h2>
          <p className="text-[#68707d] text-lg mb-6 font-medium">
          ...
          <h3 className="text-2xl m-0 mb-4 font-bold">Cómo trabajo</h3>
          <ul className="steps list-none p-0 m-0 grid grid-cols-1 gap-4">
            {features.map((feature, index) => (
              <li 
                key={index} 
                className={`flex gap-4 items-start border border-line rounded-custom p-4 bg-white fade-in-up stagger-${index + 1} ${textRef.isVisible ? 'animate' : ''}`}
              >
                <span className="icon-check inline-grid place-items-center w-7 h-7 rounded-lg border-2 border-ink font-extrabold flex-shrink-0" aria-hidden="true">
                  ✓
                </span>
                <div>
                  <h4 className="m-0 mb-1 font-semibold">{feature.title}</h4>
                  <p className="m-0 text-sm text-[#68707d] font-medium">{feature.description}</p>
                </div>
              </li>
            ))}
          </ul>
