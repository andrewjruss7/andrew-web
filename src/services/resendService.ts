...
export class ResendService {
  ...
  public async sendPlanStarterEmail(userData: ContactFormData): Promise<EmailResult> {
    try {
      ...
      const emailHtml = this.createPlanStarterTemplate(userData);
      
      const result = await this.resend.emails.send({
        from: 'Andrew <andrewjruss7@gmail.com>',
        to: [userData.email],
  ...
  public async sendNotificationEmail(userData: ContactFormData): Promise<EmailResult> {
    try {
      ...
      const notificationHtml = this.createNotificationTemplate(userData);
      ...
      console.log('Notificación enviada exitosamente con Resend:', result);
  ...
  public async processContactForm(userData: ContactFormData): Promise<EmailResult> {
    try {
      ...
      console.error('Error procesando formulario con Resend:', error);
      return {
        success: false,
  ...
  /**
   * Crea el template HTML para el plan Starter
   */
  private createPlanStarterTemplate(userData: ContactFormData): string {
    return `
      <div style="font-family: 'Poppins', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #ffffff;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
          <div style="width: 80px; height: 80px; border-radius: 50%; overflow: hidden; margin: 0 auto 15px auto; border: 3px solid #eceff3;">
            <img src="https://andrew-nextjs.vercel.app/andrew.jpg" alt="Andrew" style="width: 100%; height: 100%; object-fit: cover; object-position: 50% 30%;">
          </div>
        ...
        <div style="margin-bottom: 30px;">
          <h2 style="color: #0e1116; font-size: 24px; font-weight: 600; margin: 0 0 15px 0;">Hola ${userData.name},</h2>
  ...
  /**
   * Crea el template HTML para la notificación
   */
  private createNotificationTemplate(userData: ContactFormData): string {
    return `
      <div style="font-family: 'Poppins', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f6f8fb; padding: 20px; border-radius: 12px; margin: 20px 0;">
          ...
          ${userData.message ? `<p><strong>Mensaje:</strong> ${userData.message}</p>` : ''}
        </div>
